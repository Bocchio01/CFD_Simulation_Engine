...
void CFD_SCGS_BC_NoSlip_Tangetial(CFD_t *cfd, SCGS_t *scgs)
{
    // Horizontal walls
    for (uint8_t i = 0;
         ...
         i++)
    {
        cfd->engine->method->state->u->data[i][cfd->engine->mesh->n_ghosts - 1] = 2.0 * 0.0 - cfd->engine->method->state->u->data[i][cfd->engine->mesh->n_ghosts];
        cfd->engine->method->state->u->data[i][cfd->engine->mesh->n_ghosts + cfd->engine->mesh->nodes->Ny] = 2.0 * cfd->in->uLid - cfd->engine->method->state->u->data[i][cfd->engine->mesh->n_ghosts + cfd->engine->mesh->nodes->Ny - 1];
    }
    ...
    for (uint8_t j = 0;
         ...
         j++)
    {
        cfd->engine->method->state->v->data[cfd->engine->mesh->n_ghosts - 1][j] = 2.0 * 0.0 - cfd->engine->method->state->v->data[cfd->engine->mesh->n_ghosts][j];
        cfd->engine->method->state->v->data[cfd->engine->mesh->n_ghosts + cfd->engine->mesh->nodes->Nx][j] = 2.0 * 0.0 - cfd->engine->method->state->v->data[cfd->engine->mesh->n_ghosts + cfd->engine->mesh->nodes->Ny - 1][j];
    }
}
