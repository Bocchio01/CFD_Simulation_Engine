...
void CFD_Generate_Mesh(CFD_t *cfd)
{
    ...
    for (uint8_t i = 0; i < grid_dimension_x; i++)
    {
        ...
        {
            cfd->engine->mesh->data->x->data[i][j] = (i - cfd->engine->mesh->n_ghosts + 0.5) * cfd->engine->mesh->element->size->dx;
            cfd->engine->mesh->data->y->data[i][j] = (j - cfd->engine->mesh->n_ghosts + 0.5) * cfd->engine->mesh->element->size->dy;
        }
    }
}